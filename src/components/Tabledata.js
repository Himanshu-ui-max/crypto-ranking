...
export default function Tabledata(props) {
  ...
  <tbody>
    ...
    <tr className='d-flex justify-content-evenly' style={{fontSize:'1.6rem'}}>
      ...
      <td className='tabledata' style={{width : '300px',textAlign :'left' }}>${item.current_price.toFixed(2)}</td>
      <td className='tabledata' style={{width : '300px', textAlign :'left'}}>{item.price_change_percentage_24h.toFixed(2)}%</td>
      <td className='tabledata' style={{width : '300px',textAlign :'left'}}>${item.market_cap}</td>
